...
| 目标 | 后端程序 | 常用配置 | 常用前端 |
| :--- | :--- | :--- | :--- |
| 运行 BS 端 | `OFDMModulator` | `config/Modulator_X310.yaml` 或 `config/Modulator_B210.yaml` | `plot_sensing.py` / `plot_sensing_fast.py` |
| 运行 UE 端 | `OFDMDemodulator` | `config/Demodulator_X310.yaml` 或 `config/Demodulator_B210.yaml` | `plot_bi_sensing.py` / `plot_bi_sensing_fast.py` |
...
| `enable_system_delay_estimation` | `bool` | `false` | 若为 `true`，该通道会在启动时执行一次基于 ZC 相关的系统时延估计，随后继续消耗帧数据，但保持常规感知处理链停用。 |
...
* 当某通道设置 `enable_system_delay_estimation=true` 时，该通道只会在启动附近执行一次系统时延估计，后续不再重复做该计算，但会继续消耗帧数据；常规感知处理和感知 UDP 输出保持停用。
...
| `enable_bi_sensing` | `bool` | `true` | 启用双站感知处理链和 UDP 输出；设为 `false` 时 `OFDMDemodulator` 不会启动双站感知通道。 |
...
| `profiling_modules` | `string` | `""` | 性能统计模块列表，逗号分隔。常用值包括 `demodulation`、`agc`、`align`；`all` 表示全部。`agc` 控制 AGC 日志，`align` 控制运行时 `ALGN:` 日志。 |
